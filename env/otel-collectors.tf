locals {
  opentelemetry_collectors = {
    otlp = {
      ...
      env         = {
        OTLP_ENDPOINT                   = "jaeger-operator-jaeger-collector.${kubernetes_namespace_v1.jaeger.metadata[0].name}.svc.cluster.local:4317"
        PROMETHEUS_PUSHGATEWAY_ENDPOINT = "http://prometheus-server.${kubernetes_namespace_v1.prometheus.metadata[0].name}.svc.cluster.local/api/v1/write"
      }
    }
  }
}
